...
//	float   k;           /* overall gain factor */
	...
	[self adjustFilterCutoff:1200.0f resonance:1.1f];
//
//	/*
//	 * Compute z-domain coefficients for each biquad section
//	 * for new Cutoff Frequency and Resonance
...
float scaleFloat (float in, UInt8 numKeysPlaying) {
	switch (numKeysPlaying) {
		case 0:
			return in * 1.000f;
		case 1:
			return in * 1.000f;
...
float scaleToFloat (SInt16 in, UInt8 numKeysPlaying) {
	switch (numKeysPlaying) {
		case 0:
			return in / 32767.0f;
		case 1:
...
//	SInt32 *clipProofDeviceBuffer = malloc(sizeof(SInt32) * inNumberFrames);
//	memset(clipProofDeviceBuffer, 0, sizeof(SInt32) * inNumberFrames);
			...
			if((instrument->keysAreStarting >> checkedBit) & 1 == 1) {
				...
				while(remainingFramesToFill > (sampleBufferSizeFrames - sampleBufferCursor)) {
					...
					for(int i=0; i<sampleBufferSizeFrames-sampleBufferCursor; i++) {
						...
						//float rawValue = scaleToFloat(sampleBuffer[sampleBufferCursor+i], numKeysPlaying);
						float rawValue = (float) sampleBuffer[sampleBufferCursor+i] / 32767.0f;
						float fadedValue = 0.0f;
						float fadeMultiplier = powf(((float) ++fadeInFraction / kFadeStepsFloat), kFadeMultiplierExponent);
...
//						NSLog(@"1fade(%d): %.3fx :%.3f to %.3f", fadeInFraction, fadeMultiplier, rawValue, fadedValue);
						
//						floatDeviceBuffer[deviceBufferCursor+i] += fadedValue;
						floatDeviceBuffer[deviceBufferCursor+i] += scaleFloat(fadedValue, numKeysPlaying);
...
//						float rawValue = scaleToFloat(sampleBuffer[sampleBufferCursor+i], numKeysPlaying);
						...
						if(fadeInFraction < kFadeSteps) {
							fadeMultiplier = powf(((float) ++fadeInFraction / kFadeStepsFloat), kFadeMultiplierExponent);
							fadedValue = rawValue * fadeMultiplier;
						} else {
							fadedValue = rawValue;
						}
						
//						NSLog(@"2fade(%d): %.3fx :%.3f to %.3f", fadeInFraction, fadeMultiplier, rawValue, fadedValue);
				...
				// KEYS THAT ARE STOPPING
				// we need to fade them out to avoid pop
...
//						float rawValue = scaleToFloat(sampleBuffer[sampleBufferCursor+i], numKeysPlaying);
						...
						if(fadeOutFraction > 0) {
							fadedValue = rawValue * fadeMultiplier;
							//GSB 20100407 tuning click fadedValue = fadedValue * pow(((Float32) --fadeOutFraction / kFadeStepsFloat), 2);
						} else {
							fadedValue = 0.0f;
							noteHasStopped = YES;
							sampleBufferCursor = 0;
						}
//						NSLog(@"3fade(%d): %.3fx :%.3f to %.3f", fadeOutFraction, fadeMultiplier, rawValue, fadedValue);
...
//						NSLog(@"4fade(%d): %.3fx :%.3f to %.3f", fadeOutFraction, fadeMultiplier, rawValue, fadedValue);
				...
				while(remainingFramesToFill > (sampleBufferSizeFrames - sampleBufferCursor)) {
					...
					for(int i=0; i<sampleBufferSizeFrames-sampleBufferCursor; i++) {
						floatDeviceBuffer[deviceBufferCursor+i] += scaleToFloat(sampleBuffer[sampleBufferCursor+i], numKeysPlaying);
//						deviceBuffer[deviceBufferCursor+i] += sampleBuffer[sampleBufferCursor+i];
//						clipProofDeviceBuffer[deviceBufferCursor+i] += (SInt64) sampleBuffer[sampleBufferCursor+i];;
					}
...
//						clipProofDeviceBuffer[deviceBufferCursor+i] += (SInt64) sampleBuffer[sampleBufferCursor+i];
	...
	for(int i=0; i<framesToRead; i++) {
		...
		float waveLowPassPre = floatDeviceBuffer[i];
		//float waveLowPassPost = iir_filter(waveLowPassPre, instrument->iir_length, instrument->iir_history, instrument->iir_coef);
		float waveLowPassPost = waveLowPassPre;
		
		if(waveLowPassPre < 0.99 && waveLowPassPost > 0.99) {
//			NSLog(@"!!! pre fine, but waveLowPassPost >0.99");
		}

		float compressPost = waveshape_distort(waveLowPassPost);

		if(compressPost > 0.99) {
//			NSLog(@"compressPost >0.99");
		}
		
//		float waveLowPassPost = waveLowPassPre;
		SInt16 valueOut = waveLowPassPost * 32767;
		
		deviceBuffer[i] = valueOut;
//		SInt16 deviceBufferOld = deviceBuffer[i];
//		SInt64 clipProofDeviceBufferValue = clipProofDeviceBuffer[i];
//		SInt16 deviceBufferNew = (SInt16) clipProofDeviceBuffer[i];
//		deviceBuffer[i] = (SInt16) clipProofDeviceBuffer[i];
	}
...
#define kTouchExpansionPixels 7
-(void) playTouchPoints {
	...
	for (CGPoint* touchPoint=begin; touchPoint!=end; ++touchPoint) {
		...
		// northwest
		if(touchScreenX-kTouchExpansionPixels >= 0 && touchScreenY-kTouchExpansionPixels >= 0) {
			UInt32 key = [self getKeyNumberForPoint:CGPointMake(touchPoint->x-kTouchExpansionPixels,touchPoint->y-kTouchExpansionPixels)];
			if(key < 32) keysShouldBePlaying |= 1 << key;
		}

		// southwest
		if(touchScreenX-kTouchExpansionPixels >= 0 && touchScreenY+kTouchExpansionPixels <= 204) {		
			UInt32 key = [self getKeyNumberForPoint:CGPointMake(touchPoint->x-kTouchExpansionPixels,touchPoint->y+kTouchExpansionPixels)];
			if(key < 32) keysShouldBePlaying |= 1 << key;
		}
		
		//  northeast
		if(touchScreenX+kTouchExpansionPixels <= 478 && touchScreenY-kTouchExpansionPixels >= 0) {
			UInt32 key = [self getKeyNumberForPoint:CGPointMake(touchPoint->x+kTouchExpansionPixels,touchPoint->y-kTouchExpansionPixels)];
			if(key < 32) keysShouldBePlaying |= 1 << key;
		}
		
		// southeast
		if(touchScreenX+kTouchExpansionPixels <= 478 && touchScreenY+kTouchExpansionPixels <= 204) {
			UInt32 key = [self getKeyNumberForPoint:CGPointMake(touchPoint->x+kTouchExpansionPixels,touchPoint->y+kTouchExpansionPixels)];
			if(key < 32) keysShouldBePlaying |= 1 << key;
		}
	}
